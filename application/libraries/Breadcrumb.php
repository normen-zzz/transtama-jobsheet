...
class Breadcrumb
{
	...
	public $template = [
		'tag_open' => ' <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">',
		'crumb_open' => '<li class="breadcrumb-item text-muted">',
		'crumb_active' => '<li class="breadcrumb-item text-muted">'
	];
	...
	public function generate()
	{
		if ($this->breadcrumb) {
			...
			foreach ($this->breadcrumb as $key => $value) {
				...
				// 	$out .= '<span class="text-muted">' . $page . '</span>';
				...
				if (end($keys) == $key) {
					...
					$out .= $this->template['crumb_open'];
					$out .= anchor($value['href'], $value['page'], 'class="text-muted"');
					$out .= $this->template['crumb_close'];
					$out .= $this->newline;
				}
				// }
