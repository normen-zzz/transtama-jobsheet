  ...
  public function addWeight($id)
    {
        ...
        for ($i = 0; $i < sizeof($panjang); $i++) {
            ...
            $beratVolume = ($panjang[$i] * $lebar[$i] * $tinggi[$i]) / $pembagi;
            $beratVolumeSetelahKoma = $beratVolume - floor($beratVolume);
            if ($beratVolumeSetelahKoma >= 0.3) {
                $volume = ceil($beratVolume); // Pembulatan ke atas
            } else {
                $volume = floor($beratVolume); // Pembulatan ke bawah
            }
            ...
            if ($no_do[$i] != NULL) {
                ...
                if ($do['koli'] != NULL) {
                    ...
                    $insertKoliDo = $this->db->update('tbl_no_do', array('koli' => ($do['koli'] + $koli[$i])), array('shipment_id' => $shipment_id, 'no_do' => $no_do[$i]));
                } else {
                    $insertKoliDo = $this->db->update('tbl_no_do', array('koli' => $koli[$i]), array('shipment_id' => $shipment_id, 'no_do' => $no_do[$i]));
                }

                if ($do['berat'] != NULL) {  //jika berat di setiap do null

                    if ($volume > $berat[$i]) {
                        $insertBeratDo = $this->db->update('tbl_no_do', array('berat' => ($do['berat'] + ($volume * $koli[$i]))), array('shipment_id' => $shipment_id, 'no_do' => $no_do[$i]));
                    } else {
                        $insertBeratDo = $this->db->update('tbl_no_do', array('berat' => ($do['berat'] + ($berat[$i] * $koli[$i]))), array('shipment_id' => $shipment_id, 'no_do' => $no_do[$i]));
                    }
                } else {
                    if ($volume > $berat[$i]) {
                        $insertBeratDo =  $this->db->update('tbl_no_do', array('berat' => ($volume * $koli[$i])), array('shipment_id' => $shipment_id, 'no_do' => $no_do[$i]));
                    } else {
                        $insertBeratDo =  $this->db->update('tbl_no_do', array('berat' => ($berat[$i] * $koli[$i])), array('shipment_id' => $shipment_id, 'no_do' => $no_do[$i]));
                    }
                }
            }
           $insertDimension =  $this->db->insert('tbl_dimension', $data);
        ...
        if ($update && $insertBeratDo && $insertKoliDo && $insertDimension) {
